...
async function getAcrithisMessage() {
  try {
    const now = Date.now()
    const utc = new Date()
    const nextDaily = Date.UTC(utc.getUTCFullYear(), utc.getUTCMonth(), utc.getUTCDate() + 1)
    const day = utc.getUTCDay()
    const daysUntilMon = (8 - day) % 7 || 7
    const nextWeekly = Date.UTC(utc.getUTCFullYear(), utc.getUTCMonth(), utc.getUTCDate() + daysUntilMon)

    const PRICE = {
      'Orokin Reactor': 20, 'Orokin Catalyst': 20,
      'Exilus Warframe Adapter': 20, 'Exilus Weapon Adapter': 20,
      'Primary Arcane Adapter': 25, 'Secondary Arcane Adapter': 25,
      'Forma': 10, 'Melee Riven Mod': 15, 'Pistol Riven Mod': 15,
      'Rifle Riven Mod': 15, 'Kitgun Riven Mod': 15, 'Zaw Riven Mod': 15,
      'Shotgun Riven Mod': 15, 'Companion Weapon Riven Mod': 20,
      '5000 Kuva': 10
    }

    const url = 'https://wiki.warframe.com/api.php?' + new URLSearchParams({
      action: 'parse', page: 'Acrithis/Current_Offerings',
      prop: 'wikitext', format: 'json', origin: '*'
    })
    const res = await axios.get(url, { timeout: 15000 })
    const wikitext = res.data && res.data.parse && res.data.parse.wikitext && res.data.parse.wikitext['*']
    if (!wikitext) return '❌ Não foi possível ler a rotação.'
    ...
    if (!items.length) reply += '_Sem dados da wiki_\n'
